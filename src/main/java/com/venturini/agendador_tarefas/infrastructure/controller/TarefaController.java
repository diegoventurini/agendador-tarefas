...
public class TarefaController {
    ...
    @PostMapping
    public ResponseEntity<TarefaDTORecord> gravarTarefa(@RequestBody TarefaDTORecord tarefaDTO,
                                                  @RequestHeader("Authorization") String token) {
        return ResponseEntity.ok(tarefaService.gravaTarefa(tarefaDTO, token));
    }

    @GetMapping("/eventos")
    public ResponseEntity<List<TarefaDTORecord>> buscarListaTarefaAgendadaPorPeriodo(
    ...
    @GetMapping
    public ResponseEntity<List<TarefaDTORecord>> buscarListaTarefaPorEmail(
            @RequestHeader("Authorization") String token) {
    ...
    @PatchMapping
    public ResponseEntity<TarefaDTORecord> alterarStatusNotificacao(@RequestParam("status")StatusNotificacaoEnum statusNotificacaoEnum,
                                                              @RequestParam("id") String id) {
        return ResponseEntity.ok(tarefaService.alteraStatusNotificacao(statusNotificacaoEnum, id));
    }

    @PutMapping
    public ResponseEntity<TarefaDTORecord> atualizarTarefas(@RequestBody TarefaDTORecord tarefaDTO,
                                                      @RequestParam("id") String id) {
